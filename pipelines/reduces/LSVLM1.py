from pipelines.exp_config import SENTENCE_START, SENTENCE_END, PHRASE_DELIMITER
    ...
    for item in itemiterator:
        ...
        if not doc_attr is None:
            if content_format == "text":
                ...
                item_out.append(item.get_attribute(doc_attr, Sentences1))
        if not cap_attr is None:
            if content_format == "text":
                item_out.append(item.get_attribute(cap_attr))
            elif content_format == "sentences":
                item_out.append(item.get_attribute(cap_attr, Sentences1))
        if content_format == "text":
