from pipelines.formats.LengthModels1 import *
...
def get_text_lengths_for_latex(itemiterator, doc_attr):
	...
	for item in itemiterator:
		lengths.append(len(item.get_attribute(doc_attr, Sentences1).get_text().split()))	
...
def get_sentence_lengths_for_latex(itemiterator, doc_attr = None, cap_attr = None):
	...
	for item in itemiterator:
		if doc_attr:
			lengths.extend(item.get_attribute(doc_attr, Sentences1).get_sentence_lengths())	
		if cap_attr:
			lengths.extend(item.get_attribute(cap_attr, Sentences1).get_sentence_lengths())	
	for l in lengths:
		print l
...
def plot_sentence_lengths(itemiterator, doc_attr = None, cap_attr = None, name = "Sentence lengths", maxval=None):
    ...
    for item in itemiterator:
        if doc_attr:
            sentlens = item.get_attribute(doc_attr, Sentences1).get_sentence_lengths()
            for l in sentlens:
                lengths.append(l)
        if cap_attr:
            sentlens = item.get_attribute(cap_attr, Sentences1).get_sentence_lengths()
            for l in sentlens:
                lengths.append(l)
    import pylab as PLT
    PLT.figure()
